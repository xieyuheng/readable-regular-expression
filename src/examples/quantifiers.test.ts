...

{
  ...
  assert(
    util.equal(
      modified_quote.match(non_greedy),
...

{
  ...
  assert(
    util.equal(
      sentence.match(single_letter_word),
      ["I"]))

  assert(
    util.equal(
      sentence.match(not_so_long_word),
      ["Why", "do", "I", "have", "to", "learn", "table"]))

  assert(
    util.equal(
      sentence.match(loooong_word),
      ["multiplication"]))
}
